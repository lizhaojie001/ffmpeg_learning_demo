...
void VideoPlayer::readVideoPkt()
{
    while (true)
    {
        ...
        //格式转换
        ret = sws_scale(_vSwsCtx, _vSwsInFrame->data, _vSwsInFrame->linesize, 0, _vCodecCtx->height, _vSwsOutFrame->data, _vSwsOutFrame->linesize);
        if (ret <= 0)
        {
            continue;
        }
...
int VideoPlayer::initSws()
{
    ...
    _vSwsOutSpec.size = av_image_get_buffer_size(_vSwsOutSpec.pix_fmt, _vSwsOutSpec.width, _vSwsOutSpec.height, 1);
    _vSwsCtx = sws_getContext(_vCodecCtx->width, _vCodecCtx->height, _vCodecCtx->pix_fmt, _vSwsOutSpec.width, _vSwsOutSpec.height, _vSwsOutSpec.pix_fmt, SWS_BILINEAR, nullptr, nullptr, nullptr);
    if (!_vSwsCtx)
    {
    ...
    int ret = av_image_alloc(_vSwsOutFrame->data, _vSwsOutFrame->linesize, _vSwsOutSpec.width, _vSwsOutSpec.height, _vSwsOutSpec.pix_fmt, 1);
    RET(av_image_alloc);
