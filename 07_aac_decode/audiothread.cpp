...
spec.filename = DECODEPCMFILENAME;
#else
    EncodeAudioSpec spec;
...
outSpec.filename = outFIle;
FFmpegs::aacDecode(outSpec,spec);
qDebug() << "比特率:" << spec.sample_rate;
qDebug() << "格式:" << av_get_sample_fmt_name(spec.sample_format);
qDebug() << "声道数:" << spec.chLayout.nb_channels;
